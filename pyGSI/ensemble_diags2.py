            ...
            for expt_name in expt_names:
                ...
                while mem <= n_mem:
                    ...
                    if mem == 1:
                        ...
                        # Digitize itot - Before "pressure = pressure[used]"
                        ...
                        # Digitize iasm, errorinv (calculate means so divide by iasm)
                        pindx = _np.digitize(pressure, pbins) - 1
                        iasm, bin_edges = _np.histogram(pressure, pbins[::-1])
                        #error = _np.bincount(pindx, minlength=n_levs, weights=error)
                        # end if mem==1
                ...
                # Digitize vars
                fcst_ens = _np.bincount(pindx, minlength=n_levs, weights=fcst_ens)
